  ...
  return (
    <div className={`relative flex items-center ${className}`}>
      <button
        ...
        aria-label={isConnected ? 'Disconnect Device' : isConnecting ? 'Connecting to Device' : 'Connect Device'}
        aria-busy={isConnecting}
        type="button"
        className={`group flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all shadow-sm hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 ${isConnected ? 'bg-green-500/15 text-green-700 dark:text-green-300 hover:bg-green-500/25 border border-green-400/40' : 'bg-blue-500/15 text-blue-700 dark:text-blue-300 hover:bg-blue-500/25 border border-blue-400/40'}`}
        disabled={isConnecting}
      >
        {isConnecting ? (
          <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none" role="status" aria-label="Loading"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.333 0 1 4.333 1 12h3z" /></svg>
        ) : (
